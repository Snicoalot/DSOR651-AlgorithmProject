''' 
This file contains a harder version of tic tac toe.
Instead of the classic 3x3 grid, the player is tasked to win with the same rules as the 3x3 grid (any 3 tokens in a row)
while playing on a 4x4 grid. Good luck, you'll need it!

As an edit, if you aren't a moron, whoever goes first wins :P
'''
...
def checkWin(board, player):
    ...
    winningPositions = [
        [0, 1, 2], [1, 2, 3], [4, 5, 6], [5, 6, 7], [8, 9, 10], [9, 10, 11], [12, 13, 14], [13, 14, 15], # Rows
        [0, 4, 8], [4, 8, 12], [1, 5, 9], [5, 9, 13], [2, 6, 10], [6, 10, 14], [3, 7, 11], [7, 11, 15], # Columns
        [0, 5, 10], [5, 10, 15], [1, 6, 11], [4, 9, 14], # Diagonals
...
def getMove(board):
    ...
    while True:
        print(f"Enter your move (0-15): ")
        move = input("")
        # Will not accept non-integer numbers, or numbers in illegal placements
        try:
            move = int(move)
            if isinstance(move, int):
                if 0 <= move <= 15 and board[move] == 0:
                    return move
                else:
...
def printBoard(board):
    ...
    for i in range(0, 16, 4):
        print(f" {' '.join(['X' if board[j] == 1 else ('O' if board[j] == -1 else '_') for j in range(i, i+4)])}")
    print("\n")
